...
function SeeCommentsOnDoc() {
    ...
    let get_data = async () => {
        ...
        if (role == 'CoordinateCommitte') {
            let response = await axios.post('http://localhost:5000/get_comments_for_commetti_members', {
                pdf_id: pdf_id
            })
        ...
        if (role == 'student' || role == 'supervisor') {
            ...
            const _data = await Promise.all(
                comment?.map(async (com, index) => {
                    ...
                    if (teacher.role.some((item)=>item==role)) {
                        com['teacher_name'] = teacher.name
                ...
                <div className="flex flex-col flex-1">
                    ...
                    <div className="flex-1 p-4 min-h-0 overflow-auto">
                        <Typography variant="body2" sx={{ marginBottom: '10px' }}>
                            {'Document'} : {pdfdata?.document_name}
                        </Typography>
                            ...
                            {comment?.length > 0 && (
                                <>
                                    {comment.map((item) => {
                                        return (
                                            <>
                                                <Grid sx={{ display: 'flex', justifyContent: 'start', marginTop: 1 }}>
                                                    <Card
                                                        variant="outlined"
                                                        sx={{
                                                            width: '100%',
