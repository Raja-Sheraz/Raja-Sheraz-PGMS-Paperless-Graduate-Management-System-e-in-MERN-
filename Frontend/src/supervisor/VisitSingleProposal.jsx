...
function VisitSingleProposal() {
    ...
    useEffect(() => {
        ...
        if (sessionStorage.getItem('logged_user_role') == 'CoordinateCommitte') {
            get_all_dacs()
        }
    }, [viewPdf, pdfid])
    ...
    const Feedback = async (text) => {
        if (modify) {
            try {
                if (
                    sessionStorage.getItem('logged_user_role') == 'CoordinateCommitte' ||
                    sessionStorage.getItem('logged_user_role') == 'DAC'
                ) {
                    modify_paper(text)
                } else if (sessionStorage.getItem('logged_user_role') == 'dean') {
                    let response = await axios.post('http://localhost:5000/dean_modify', {
                        pdf_id: pdf_id,
                        reason: text
                    })
            ...
            try {
                if (
                    ...
                    let response = await axios.post('http://localhost:5000/dean_reject', {
                        pdf_id: pdf_id,
                        reason: text
                    })
                    toast.success(response.data.result)
        ...
        if (action === 'Reject') {
            ...
            try {
                if (
                    ...
                    let response = await axios.post('http://localhost:5000/dean_approve', {
                        pdf_id: pdf_id
                    })
                    toast.success(response.data.result)
                    navigate('/viewPapers')
    ...
    const [commettihead,setcommettihead]=useState(null);


    const getCommettiMembers=async()=>{
        const response=await axios.post("http://localhost:5000/get_commetti_members",{
            pdf_id:pdf_id
        });
        console.log(response.data.head)
        setcommettihead(response.data.head)
    }
                            ...
                            <div className="col-12 col-lg-8">
                                <div>
                                    ...
                                    <div className="d-flex flex-wrap justify-content-center my-2">
                                        ...
                                        {sessionStorage.getItem('logged_user_role') == 'Supervisor' &&
                                            pdfdata &&
                                            pdfdata.pdf_type != 'application' && (
                                                <>
                                                    <button
                                                        type="button"
                                                        className="btn btn-warning"
                                    ...
                                    {sessionStorage.getItem('logged_user_role') == 'CoordinateCommitte' && user?._id==commettihead &&(
                                        <>
